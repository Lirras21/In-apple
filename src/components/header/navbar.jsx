          ...
          <div className='lg:flex hidden items-center h-[100px] relative w-[704px]'>
            {items.map((item, index) => (
              ...
              >
                ...
                <div className={`${activeIndex === index ? 'absolute top-[99px] w-40 ' : 'hidden'} bg-slate-100 left-0`}>
                  <ul className='mt-4 mb-4'>
                    {item.list.map((el, i) => (
                      <li key={i} className='h-7 ml-4'>
                        <a href="" className='relative after:bg-black after:absolute after:h-[1px] after:w-0 after:bottom-0 after:left-0 hover:after:w-full after:transition-all after:duration-300 cursor-pointer'>{el}</a>
                      </li>
                    ))}
                  </ul>
                </div>
